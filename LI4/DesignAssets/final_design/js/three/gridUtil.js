...
/**
 * Create a plot with the specified number of blocks and block size.
 * 
 * @param {number} blocks 
 * @param {number} blockSize 
 * @param {THREE.Vector3} rotation 
 * @param {THREE.Vector3} position 
 * @param {{ x: number, z: number }} colors 
 * @returns 
 */
function makePlot(blocksX, blocksZ, blockSize, rotation, position, colors = { x: 0xffff00, z: 0x00ffff }) {
    const gridGroup = new THREE.Group();

    const GRID_SIZE_X = blocksX * blockSize;
    const GRID_SIZE_Z = blocksZ * blockSize;
    const gridEdgePosX = -(GRID_SIZE_X / 2);
    const gridEdgePosZ = -(GRID_SIZE_Z / 2);

    const xLineMaterial = new THREE.LineBasicMaterial({ color: colors.x ?? 0xffff00 });
    const zLineMaterial = new THREE.LineBasicMaterial({ color: colors.z ?? 0x00ffff });
    for (let i = 0; i <= blocksX; i++) {
        const xLineGeometry = new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(0, 0, 0), new THREE.Vector3(0, 0, GRID_SIZE_Z)]);
        const xLine = new THREE.Line(xLineGeometry, xLineMaterial);

        xLine.position.x = gridEdgePosX + i * blockSize;
        xLine.position.y = -(blockSize / 2);
        xLine.position.z = gridEdgePosZ;

        gridGroup.add(xLine);
    }

    for (let i = 0; i <= blocksZ; i++) {
        const zLineGeometry = new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(0, 0, 0), new THREE.Vector3(GRID_SIZE_X, 0, 0)]);
        const zLine = new THREE.Line(zLineGeometry, zLineMaterial);

        zLine.position.x = gridEdgePosX;
        zLine.position.y = -(blockSize / 2);
        zLine.position.z = gridEdgePosZ + i * blockSize;
...
export { makePlot, makeGrid, make3DGrid };
